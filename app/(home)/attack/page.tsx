...
const Page = () => {
  ...
  const [typeAttack, setTypeAttack] = useState<string>(
    attackTypeSystem[0].value
  );
  ...
  const generateInitialValues = useCallback(
    (settings: ISettingItem[]) => {
      ...
      const typeAttackSelect = settings.filter(
      ...
      typeAttackSelect.forEach((setting) => {
        if (setting.input === "select" && setting.value.length > 0) {
          ...
          setting.group.toLowerCase() === "option" &&
  ...
  useEffect(() => {
    ...
    const modeSetting = finalSetting.find(
      (s) => s.group.toLowerCase() === "mode"
    );

    try {
      const requestSetting = finalSetting.find(
        (s) => s.group.toLowerCase() === "request"
      );
      if (requestSetting && form.getFieldValue(requestSetting.group)) {
        setRequestValue(form.getFieldValue(requestSetting.group));
      }

      const concurrentSetting = finalSetting.find(
        (s) => s.group.toLowerCase() === "concurrents"
      );
      ...
      if (modeSetting) {
        ...
        if (modeValue) {
          const selectedMode = modeSetting.value.find(
            (v) => v.value === modeValue || v.key === modeValue
          );
          if (selectedMode) {
            const isBladeMode = selectedMode.label
              .toLowerCase()
              .includes("blade");
            const isDeathWorkMode = selectedMode.label
              .toLowerCase()
              .includes("death sword");
  ...
  const renderGroupedSetting = () => {
    ...
    const concurrentsGroup = finalSetting.find(
      (setting) => setting.group.toLowerCase() === "concurrents"
    );

    const totalThreads = concurrentsGroup
      ? Math.max(
          ...concurrentsGroup.value.map((item) =>
            typeof item.value === "string"
              ? parseInt(item.value, 10)
              : item.value
          )
    ...
    return (
      <>
        {filteredSettings.map((setting, index, array) => {
          ...
          const isLastItemInOddArray =
            index === array.length - 1 && array.length % 2 !== 0;

          const marks: Record<number, string> =
            setting.input === "slider" && !isRequestGroup
          ...
          return (
            <div
              ...
              className={`flex flex-col card ${
                isLastItemInOddArray ? "col-span-2" : ""
              }`}
            >
              <div className="font-bold text-primary text-[30px] leading-[normal] mb-3">
                {convertToUppercaseWords(setting.group)}
              </div>
              {setting.description && (
                <div
              ...
              {setting.input === "select" && (
                <Form.Item name={setting.group}>
                  <Select
                    ...
                    onChange={(value) => {
                      if (isModeGroup) {
                        const selectedMode = setting.value.find(
                          (v) => v.value === value || v.key === value
                        );
                        if (selectedMode) {
                          const isBladeMode = selectedMode.label
                            .toLowerCase()
              ...
              {setting.input === "toggle" && (
                <div className="flex gap-5">
                  {Object.entries(
                    setting.value.reduce(
                      (
                        acc: Record<string, { label: string; value: boolean }>,
                        option
                      ) => {
    ...
    >
      ...
      {user?.isAdmin && (
        <div className="flex flex-col card col-span-2">
          <>
            <div className="font-bold text-primary text-[30px] leading-[normal] mb-2">
              SERVERS
            </div>
  ...
  const getSelectedLabel = () => {
    const selected = attackTypeSystem.find(
      (state) => state.value === typeAttack
    );
    ...
    <div className="inner-body">
      {error && (
        <NoticeError
          error={error}
          setError={setError}
          myClass="text-5xl mb-0 leading-[3.5rem]"
        />
      )}
            ...
            <div className="mx-auto p-6">
              <div className="h-full flex gap-x-6">
                <div className="h-full p-4 flex flex-col w-[525px] min-w-[525px] max-w-[525px] fixed t-0 l-0 z-10">
                  ...
                  <div className="mb-2">
                    <h1 className="text-center text-primary text-2xl font-extrabold mb-4">
                      STATE: {getSelectedLabel()}
                    </h1>
                    <div className="flex flex-col card">
                      <div className="text-base text-primary mb-2">
                        Consider between <code>HYNA Valkyra</code> and{" "}
                        <code>HYNA Warrior</code> for the most effective attack
                      </div>
                  ...
                  <button
                    ...
                    className="font-black py-3 bg-primary float-end text-black text-4xl rounded transition-all duration-300 ease-in-out active:opacity-10 hover:shadow-md hover:shadow-[#00ff00] flex items-center justify-center"
                  >
                    {attackLoading ? (
                ...
                <div
                  ...
                  style={{ marginLeft: "525px", width: "calc(100% - 525px)" }}
                >
